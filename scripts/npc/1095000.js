...
function action(mode, type, selection) {   		
        if (status == 0) {
			...
			if(mode < 1){
				...
				if(status == 1){
				cm.sendSimple("\t\t\t\t\t\t\t\t\t\t#r#eJump Quests#n#k\r\n\tGo ahead and choose a jumpquest! " + 
				"\r\n#L0##bRandom#k#l \r\n\r\n" +
				"#L1#Ghost Chimney#l" +
				"\t\t\t" +
				"#L2#  Space Gaga#l \r\n" +
			    ...
			    status = 4;
					...
					if(starter == 2 || starter == 7 || starter == 9 || starter == 11  || starter == 29)
						...
						limit = starter+7;
					msg = "\t\t\t\t\t\t\t\t\t\t#r#eJump Quests#n#k\r\nPlease choose out a jq from the group!\r\n"
	        ...
	        msg+= "\t\t\t\t\t\t\t\t\t\t#r#eJump Quests#n#k\r\n\r\n" + cm.JQrank(chosen) + cm.JQpersrank(chosen) + "#b\r\n#L100# Start JQ#l";
			cm.sendSimple(msg);
			status++;
			}
			else if(status == 4){
