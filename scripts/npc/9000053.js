...
function action(mode, type, selection) {
    ...
    if (mode == -1 || selection == 1000000) {
        ...
        if (status == 0) {
            cm.sendNext("We have plenty of fun stuff that we can trade with you. These are premium items that you can buy with event points. You earn event points by playing in games with the community and winning.");
        } else if (status == 1 || selection == 10000) {
            status = 1;
            disptype = 0;
            var selStr = "Which kind of premium item would you like to see?\r\n";
            selStr += "\r\n#L17##rSearch Item#k" +
            ...
            if ((selection < 14 && disptype == 0) || selection == 10001) {
                ...
                var selStr2 = "#eSelect which page of " + equiptype + " you wish to view.#n\r\nThere are exactly " + arrayofitems.length + " items to choose from.\r\n";
                ...
                equiptype = "";
                status = 299;
                cm.sendGetText("#ePlease enter the item name you want below, make sure you enter the name correctly (capital letters count).#n");
                return;
            } else if (selection == 17) {
                status = 1;
                cm.sendSimple("How would you like to search the desired item?#b"
                            + "\r\n#L15#Search by ID#l"
                ...
                cm.sendSimple("You have #b" + cm.getPlayer().getParticipationPoints() + " PP (Participation Points)#k\r\nIt costs #b" + ppCost + " PP#k to buy #b1 EP#k, what would you like to do?" 
                ...
                status = 300;
                var selStr2 = "There are exactly " + arrayofitems.length + " items to choose from.\r\n";
            ...
            if (checkSpots()) {
                var selStr4 = "#eDo you wish to shop again?#n\r\n\r\n#b#L10000#Yes, from the start page.#l";
                if (blue == 1) {
            ...
            var troxiedmadethis = "I have found " + allitemsarrayed.length + " results for the search '" + cm.getText() + "'\r\n";
            ...
            var text1 = "The item you are searching for does not exist, we strongly recommend you suggest the item to be added - on the forums.\r\n#L10000#Ok.";
            for (var i9=0; i9<allids.length; i9++) {
                for (var j9=0; j9<allids[i9].length; j9++) {
                    if (allids[i9][j9] == itemid) {
                        whatType = i9;
                        text1 = "#eYou have chosen the following item:#n\r\n#b#t" + itemid + "##k (#v" + itemid + "#)\r\n"
                              + "\r\n#L1##bBuy with#k - #r" + epCost[whatType] + " Event Points#k#l";
                            if (cm.getPlayer().getDonationpoints() > 0)
                                text1 += "\r\n#L2##bBuy with#k - #r" + dpCost[whatType] + " Donation Points#k#l";
            ...
            if (cm.getPlayer().getParticipationPoints() >= ppCost) {
                ...
                cm.sendOk("You don't have enough Participation Points.");
                cm.dispose();
            }   
        } else if (status == 7) {
            cm.voteMSG();
